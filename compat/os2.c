...
klibc06_npipe(int fd[2])
{
  ...
  */
  ...
  sprintf(np_name, NPS_ID, (unsigned)getpid(), (unsigned)_gettid(), pn);
...
callback_prog(const char *path, void *extra)
{
  ...
  if (*progext == '.') {
    ...
    for(i=0; exttbl[i].ext_type != 0; i++) {
      strcpy(progext, exttbl[i].ext_str);
      rc = (is_file(s) >= 0);
      if (rc) break;
    }
  }
  if (rc > 0 && extra) {
	...
	strncpy(cp->buffer, s, cp->limit_buffer - 1);
	cp->buffer[cp->limit_buffer - 1] = '\0';
      }
    }
    else
      cp->buffer = strdup(s);
