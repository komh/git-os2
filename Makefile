...
ifneq (,$(findstring OS/2,$(uname_S)))
	pathsep = ;
	NO_PREAD = YesPlease
	NEEDS_SOCKET = YesPlease
	NO_MMAP = UnfortunatelyYes
	NO_IPV6 = UnfortunatelyYes
	NEEDS_CRYPTO_WITH_SSL = YesPlease
	NO_LIBGEN_H = YesPlease
	NO_SYMLINK_HEAD = YesPlease
#	NO_SETENV = YesPlease
...
ifdef NO_CURL
	...
	curl_check := $(shell (echo 070908; $(CURL_CONFIG) --vernum) | sort -r | sed -ne 2p)
...
$(BUILT_INS): git$X
	$(QUIET_BUILT_IN)$(RM) $@ && \
	$(LN_X) git$X $@ 2>/dev/null || \
	$(LN_S_X) git$X $@ 2>/dev/null || \
	$(CP_X) git$X $@
